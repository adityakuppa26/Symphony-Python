...
class JiraRequirementsConfig(BaseModel):
    ...
    # Preserve older workflow behavior unless explicitly disabled by the workflow.
...
class WorkspaceConfig(BaseModel):
    ...
    managed_repositories: list[Path] = Field(default_factory=list)

    @field_validator("managed_repositories")
    @classmethod
    def safe_managed_repositories(cls, value: list[Path]) -> list[Path]:
        normalized: list[Path] = []
        seen: set[str] = set()
        for repository in value:
            if repository.is_absolute() or ".." in repository.parts:
                raise ValueError(
                    "workspace.managed_repositories entries must be safe relative paths"
                )
            identity = repository.as_posix().strip()
            if not identity or identity == ".":
                raise ValueError(
                    "workspace.managed_repositories entries must name a repository"
                )
...
class HooksConfig(BaseModel):
    ...
    # Legacy configuration input; verification never gates handoff.
    verify_required: Literal[False] = False
    use_development_verification_request: bool = False
    after_run: str | None = None
    timeout_seconds: int = 900

    @field_validator("verify_required", mode="before")
    @classmethod
    def advisory_verification(cls, value: Any) -> bool:
        return False
...
class WorkflowConfig(BaseModel):
    ...
    state_path: Path = Path(".symphony/symphony.sqlite3")
...
def load_config(
    ...
    config.state_path = resolve_path(config.state_path, workflow_dir, environ)
