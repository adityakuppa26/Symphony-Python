...
class OrchestratorTests(unittest.TestCase):
    ...
    def test_default_codex_runner_excludes_configured_jira_credential_names(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "symphony_jira.orchestrator.CodexRunner"
            ) as runner_class:
    ...
    def test_single_issue_execution_uses_fake_jira_and_fake_codex(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.run.verification_status, "not_configured")
    ...
    def test_failed_verify_hook_is_advisory(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            orchestrator, verification_manager, _, jira = create_verification_case(
                root,
            ...
            self.assertEqual(len(verification_manager.calls), 1)
    ...
    def test_required_verification_receives_exact_plan_repositories(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            result, verification_manager, _ = run_verification_case(
                root,
            ...
            self.assertEqual(len(verification_manager.calls), 1)
            self.assertEqual(
                verification_manager.calls[0]["affected_repositories"],
                ("repo",),
            )
    ...
    def test_completed_review_code_changes_force_review_and_complete_linked_action(
        ...
        async def run() -> None:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(completed.verification_status, "not_configured")
...
""",
            ...
            self.assertIn(
                "Only the root Description and configured Acceptance Criteria",
...
""",
            ...
            self.assertEqual(runner.prompts_seen, ["plan"])
            self.assertIn(clarification, runner.plan_prompt)
            self.assertIn("Accumulated human input for all phases", runner.plan_prompt)
...
class RecordingVerificationWorkspaceManager(WorkspaceManager):
    def __init__(self, config, hooks, status: str) -> None:
    ...
    async def run_hook(
        ...
        if name == "after_run":
            drift_path = Path(workspace_path) / "repo" / "after-run-drift.txt"
            drift_path.parent.mkdir(parents=True, exist_ok=True)
            drift_path.write_text("after-run\n", encoding="utf-8")
            log_path = Path(workspace_path) / ".symphony" / "hooks" / "after_run.log"
            log_path.parent.mkdir(parents=True, exist_ok=True)
            log_path.write_text("after-run\n", encoding="utf-8")
            return HookResult(
                name="after_run",
                returncode=0,
                log_path=log_path,
        ...
        log_path = Path(workspace_path) / ".symphony" / "hooks" / "verify.log"
        log_path.parent.mkdir(parents=True, exist_ok=True)
        log_path.write_text(
            f"fake verification status: {self.status}\n",
            encoding="utf-8",
        )
        return HookResult(
            name="verify",
            ...
            log_path=log_path,
            output=f"fake verification status: {self.status}",
        )


class SequenceVerificationWorkspaceManager(
    RecordingVerificationWorkspaceManager
):
    def __init__(
        self,
        config,
        ...
        *,
        ...
        super().__init__(config, hooks, statuses[0])
    ...
    async def run_hook(
        ...
        if name == "verify":
            ...
            if self.events is not None:
                self.events.append(f"verification_{self.status}")
        return await super().run_hook(
            name,
            script,
            workspace_path,
...
""",
    ...
    content = content.replace(
        "  strategy: hook_only",
        "  strategy: hook_only\n"
        f"  managed_repositories: [{json.dumps(workspace_subdir)}]",
    )
    verification_yaml = "hooks:\n"
    if required:
        verification_yaml += "  verify_required: true\n"
    verification_yaml += "  verify: |\n    echo verify\n"
    content = content.replace("agent:\n", verification_yaml + "agent:\n")
    content = content.replace(
        '  active_statuses: ["To Do"]',
        '  active_statuses: ["To Do"]\n  handoff_status: Done',
    )
    ...
    jira = FakeJira(issue)
    store = Store(root / "db.sqlite3")
    verification_manager = RecordingVerificationWorkspaceManager(
    ...
    orchestrator = SingleIssueOrchestrator(
        ...
        store,
        codex_runner=runner,
        workspace_manager=verification_manager,
    )
    return orchestrator, verification_manager, runner, jira
...
def run_verification_case(
    ...
    *,
    ...
    result = asyncio.run(orchestrator.run_once("T-1"))
    return result, verification_manager, jira
